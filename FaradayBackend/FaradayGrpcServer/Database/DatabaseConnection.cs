...
namespace FaradayGrpcServer.Database
{
    public class DatabaseConnection
    {
        ...
        {
            string query = $"SELECT * FROM Cars c INNER JOIN Locations l ON c.fk_location_id = l.id WHERE l.id = '{location.Id}' AND c.available = 1;";
            if (connection.State != System.Data.ConnectionState.Open)
            {
                this.OpenConnection();
            }
        ...
        public List<BookingModel> GetAllActiveBookingModels()
        {
            string query = $"SELECT " +
                ...
                $"locd.id, locd.address, locd.city, locd.zipcode, locd.country, " +
                $"car.id, locc.id, locc.address, locc.city, locc.zipcode, locc.country, " +
                $"car.price, car.brand, car.model, car.color, car.doors, car.animals_allowed, " +
                $"car.available, car.car_type, car.fuel_type, car.license_plate, " +
                $"cust.id, cust.first_name, cust.last_name, cust.gender, cust.drivers_license " +
                $"FROM Bookings b " +
                $"INNER JOIN Locations locp ON b.fk_pickup_location_id = locp.id " +
                $"INNER JOIN Locations locd ON b.fk_dropoff_location_id = locd.id " +
