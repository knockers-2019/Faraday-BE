...
namespace FaradayBEClient
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            using (var requestAllCars = bookingClient.GetAllCarModels(emptyRequest))
            {
                while (await requestAllCars.ResponseStream.MoveNext())
                {
                    car = requestAllCars.ResponseStream.Current;
